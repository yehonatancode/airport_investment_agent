...
PRECOMPUTED_CACHE_PATH = Path(__file__).parent / "airport_stats_cache.json"
_precomputed_cache: dict[str, dict] | None = None


def _load_precomputed_cache() -> dict[str, dict]:
    """Loads airport_stats_cache.json's "airports" map once (module-level
    memo, not per-call), keyed by airport code. Real bug fix: this file was
    being generated by generate_cache.py but nothing ever read it back -
    every query, cached airport or not, was hitting the live download path.
    """
    global _precomputed_cache
    if _precomputed_cache is None:
        _precomputed_cache = {}
        if PRECOMPUTED_CACHE_PATH.exists():
            with open(PRECOMPUTED_CACHE_PATH) as f:
                _precomputed_cache = json.load(f).get("airports", {})
    return _precomputed_cache


REGION_CACHE_PATH = Path(__file__).parent / "region_airports_cache.json"
_region_airports_cache: dict[str, list[str]] | None = None


def _load_region_airports_cache() -> dict[str, list[str]]:
    """Loads region_airports_cache.json's region->codes map once. Same
    cache-first pattern as _load_precomputed_cache(), for a real, separate
    cold-start cost that the per-airport cache never covered: figuring out
    "which airport codes are in this region" requires scanning the raw
    on-time-performance file's state column, which used to mean a full
    download+parse (~5-60s+ depending on data_cache/ state) just to answer
    that, before any per-airport scoring even started. Only trusted when
    its recorded period matches the current default period - region
    membership is generated by generate_region_cache.py for one fixed
    period, same as the rest of this project's data.
    """
    global _region_airports_cache
    if _region_airports_cache is None:
        _region_airports_cache = {}
        if REGION_CACHE_PATH.exists():
            with open(REGION_CACHE_PATH) as f:
                data = json.load(f)
            if data.get("period") == f"{CURRENT_YEAR}-{CURRENT_MONTH:02d}":
                _region_airports_cache = data.get("regions", {})
    return _region_airports_cache
...
def airports_in_region(
    states: set[str],
    year: int = CURRENT_YEAR,
    month: int = CURRENT_MONTH,
    region_name: str | None = None,
) -> list[str]:
    """All airport codes that operated flights from any of the given states
    this period.

    Cache-first: if region_name is given and the period matches the
    default, checks region_airports_cache.json before falling through to
    the live on-time-file scan. region_name is optional (and states is
    still required/used on a cache miss) so this stays usable for any
    ad-hoc state set, not just the named Census regions the cache covers.
    """
    if region_name is not None and year == CURRENT_YEAR and month == CURRENT_MONTH:
        cached = _load_region_airports_cache().get(region_name)
...
def build_airport_stats(airport_code: str) -> dict:
    ...
    # Cache-first: local import to avoid a circular import at module load
    # (scoring_tool imports FROM this module at the top level; by the time
    # this function actually runs, both modules are already fully loaded,
    # so this is just an attribute lookup, not a re-import).
    from scoring_tool import FORMULA_VERSION

    cached = _load_precomputed_cache().get(airport)
    if cached is not None and cached.get("formula_version") == FORMULA_VERSION:
        return cached["raw_stats"]
