...
**Bug 4 — Piper worker startup recursion (found in later exploration, not the original build).** A different kind of bug than the three above — not a scoring/math error, and not caught by hand-verifying arithmetic. `get_piper_worker()`'s call to `multiprocessing.get_context("spawn").Process().start()` at the top level of `streamlit_app.py` triggered a self-recursive process spawn: on the `spawn` start method, the child process's bootstrap unconditionally re-executes the parent's `__main__` script via `runpy` to reconstruct picklable state — and that reconstruction hit the same `Process().start()` call again, tripping Python's own "not fully bootstrapped yet" guard. This was genuinely missed by multiple automated checks first: `streamlit.testing.v1.AppTest` is independently incompatible with `spawn` (for an unrelated reason — it rebinds `sys.modules['__main__']`), and a real running server was initially checked only via `/_stcore/health`, which returns before the actual script body ever executes for a real page load. It was only caught by driving a genuine Playwright-controlled browser against a real `streamlit run` process — a real, load-bearing methodology lesson: neither a passing test-harness run nor a live-but-unexercised server is the same thing as confirming a real page load actually works. Fixed by gating the call behind `if __name__ == "__main__":` — a genuine Streamlit script execution always runs as a module literally named `"__main__"` (confirmed from Streamlit's own source), while spawn's internal bootstrap reconstruction runs it under a different name (`"__mp_main__"`), so the guard reliably tells the two apart. Note: `multiprocessing.freeze_support()` — the fix Python's own error message suggests — is a no-op on macOS/Linux (confirmed via source: it only does anything on Windows with a frozen executable) and would not have fixed this here.
...
- **Piper voices are en-US only, no regional variety.** Bot's robotic character comes from using a genuinely lower-fidelity voice tier (`en_US-danny-low`) rather than a pitch adjustment on a good voice, since Piper's `SynthesisConfig` doesn't expose native pitch control the way browser TTS did. Voice synthesis runs in an isolated subprocess (a persistent worker, not spawned per call — see below) specifically so a native crash degrades gracefully instead of taking the app down; the "Speak responses aloud" toggle defaults to off for the same reason — voice is opt-in on a fresh install rather than something a reviewer can hit by surprise.
- **Piper synthesis reliability — root-caused and substantially mitigated, not eliminated.** The earlier characterization ("intermittent across all presets") was imprecise. Direct testing shows the failure is actually deterministic by voice model, not random: Female and Male (`phoneme_type: espeak` in their model config) hit a real, confirmed upstream bug — a fixed-size stack buffer in espeak-ng's macOS build truncates its data path at native-library load time (matches the public report at OHF-Voice/piper1-gpl#272); Bot (`phoneme_type: null`) never touches that code path and has never failed in testing. Fixed the *exposure*, not the upstream bug itself: replaced spawn-per-call synthesis with a persistent worker process (`piper_worker.py`) that loads all three voices once at startup — health-checked, with retry-with-backoff (fresh process on failure, up to 3 attempts) — instead of re-triggering the risky espeak-ng init on every single request. Measured, real before/after on this exact repo: old approach, 10/10 timeouts on Female and Male across two independent test runs (a clean, deterministic 100% failure rate, not occasional); new approach, 0/32 failures across startup attempts and all three presets. The existing crash-isolation/graceful-degradation fallback ("Voice unavailable") is kept as the backstop regardless of how much more reliable this makes it.
...
- **A widget-triggered rerun can silently discard a fully-generated answer — found via direct testing, not fixed, and narrower than an earlier (incorrect) understanding of this risk.** An existing message-persistence fix appends the answer to `st.session_state.messages` immediately after the agent call returns, before any further `st.*` call — built on the assumption that a plain `st.session_state` access is itself safe from Streamlit's rerun-interrupt mechanism. Direct testing (a real Playwright-driven browser session, with file-level trace instrumentation pinpointing the exact line) shows that assumption is wrong: the interrupt can fire on a plain `st.session_state` attribute *read*, not only on an `st.*` render call. Confirmed real sequence: the agent call completes fully and correctly, the reply text is built, and the interrupt fires the moment `st.session_state.messages` is touched — before the answer is ever appended, discarding it. Triggering it requires a widget interaction (e.g. toggling "Speak responses aloud") while a query is in flight; the answer generation itself is unaffected, only its persistence. Not yet fixed.
- **Submitting a second question while the first is still in flight can crash the app outright, with a real `RuntimeError: This event loop is already running`.** `get_event_loop()` caches one `asyncio` event loop per browser session in `st.session_state.loop`; nothing in the chat UI prevents a second `run_until_complete()` call from starting on that same loop object while an earlier one is still actively executing. Confirmed via direct testing that this reproduces from simply submitting a second question a few seconds after the first, before it finishes — no widget toggle needed, and unrelated to any of today's changes. This is a pre-existing latent bug in the original chat-loop design, only now found because today's testing pushed on real concurrent-interaction scenarios for the first time. Not yet fixed.
...
Multi-airport questions were originally slow for two real, since-addressed reasons — not because tool calls themselves were sequential or slow. Direct measurement (real before/after timing, not assumption) found tool *execution* was already fast and effectively concurrent for comparison questions; the actual costs were an SDK-level tool-selection round trip (`ToolSearch`) on every turn, and a cold region-membership lookup requiring a full BTS file scan.

Fixed: `tools=[]` in `ClaudeAgentOptions` removes the entire built-in Claude Code toolset, so the model doesn't need a `ToolSearch` round trip to discover the 3 real tools this agent actually has — measured ~35-45% faster time-to-first-tool-call (isolated from narration-length noise, which otherwise dominates and misleads a total-wall-time comparison). A precomputed `region_airports_cache.json` (built by `generate_region_cache.py`) gives `airports_in_region()` a cache-first path: a real, controlled before/after on this exact repo (via `git stash`/`pop`, not just exploratory numbers) measured 4.69s → 0.0003s for the region-membership scan.

Two ideas were investigated and deliberately *not* implemented, based on real measurement rather than assumption: two-model routing (Haiku for tool orchestration, Sonnet for narration) — timing showed tool-selection was never the bottleneck (~0.7s difference), so splitting models wouldn't meaningfully help; and explicit parallel/batched tool calls — diagnosis showed comparison-question tool calls are already fast and effectively concurrent, so there was no real bottleneck left there to fix.
...
A first-time query against a new time period downloads and caches the underlying BTS files locally (`data_cache/`, gitignored, multi-GB). The committed `airport_stats_cache.json` pre-computes scores for 76 major airports so most queries resolve instantly without that wait; anything outside the cache still resolves correctly, just live. The committed `region_airports_cache.json` (built by `generate_region_cache.py`) similarly pre-computes region membership so a region-ranking question doesn't require a live file scan just to find out which airports are in it.