...
- **Piper voices are en-US only, no regional variety.** Bot's robotic character comes from using a genuinely lower-fidelity voice tier (`en_US-danny-low`) rather than a pitch adjustment on a good voice, since Piper's `SynthesisConfig` doesn't expose native pitch control the way browser TTS did. Piper's native espeak-ng dependency can also fail to initialize on some fresh installs (a known packaging quirk, not introduced by this project) — confirmed on a genuinely fresh clone, where it crashed the whole process the first time. Voice synthesis now runs in an isolated subprocess specifically so that failure mode degrades gracefully instead of taking the app down, at the cost of a ~1-2s per-response overhead for spawning that process; the "Speak responses aloud" toggle defaults to off for the same reason — voice is opt-in on a fresh install rather than something a reviewer can hit by surprise.
- **Piper synthesis also occasionally times out**, separately from the crash above, inside that same isolated subprocess — observed across all three presets in testing (Female, Male, and Bot each failed at least once and succeeded at least once across repeated trials), not specific to one voice. Root cause not identified; spawn overhead and Gatekeeper quarantine flags were checked and ruled out as likely causes. The default preset was changed to Male on the assumption it was more reliable, but further testing showed that assumption doesn't hold — the timeout is intermittent regardless of preset, and the investigation was intentionally stopped short of a full fix given submission timing. This failure mode is already handled correctly by the same isolation: the text response completes and renders normally, a "Voice unavailable" note appears in place of audio, and the app never breaks. Treated as a known, accepted limitation of a bonus feature, not a risk to the core product.
